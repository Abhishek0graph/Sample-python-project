...
def create_table_if_not_exists():
    table_name = "items_list_with_labels"
    try:
...
def upload_image_to_s3(file, file_name):
    try:
        ...
        # Generate S3 URL
        s3_url = f"https://{bucket_name}.s3.eu-west-1.amazonaws.com/{file_name}"
        return s3_url
...
#             # Step 3: Return response with labels
#             return JsonResponse({"message": "Image uploaded successfully", "image_url": image_url, "labels": labels}, status=200)
...
def add_item(request):
    if request.method == "POST":
        ...
        response = sns_client.publish(
            PhoneNumber="918054810988",  # Replace with the recipient's phone number in E.164 format
